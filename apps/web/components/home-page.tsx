...
export function HomePage({ decks }: { decks: PublicDeck[] }) {
  ...
  const featured = editorialOrder.flatMap((slug) => { const deck = decks.find((item) => item.slug === slug); return deck ? [deck] : []; }).slice(0, 4);
  ...
  return <div className="home-shell quiet-home"><SiteHeader copy={copy} locale={locale} theme={theme} onLocale={changeLocale} onTheme={changeTheme} /><main>
    <section className="pocket-home" aria-label={zh ? '试学卡片' : 'Try a card'}>
      ...
      <div className="pocket-links"><Link href="/study/arts-37-color-theory">{zh ? '继续学习' : 'Keep studying'} <span aria-hidden="true">↗</span></Link><span aria-hidden="true">·</span><Link href="/app/decks/new">{zh ? '制作我的卡片' : 'Make your own'}</Link></div>
    </section>

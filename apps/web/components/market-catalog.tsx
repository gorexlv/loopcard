...
export function MarketCatalog({ decks, embedded = false }: { decks: PublicDeck[]; embedded?: boolean }) {
  ...
  const categories = useMemo(() => [...new Set(decks.map((deck) => deck.category))], [decks]);
  const filtered = useMemo(() => {
    const needle = query.trim().toLowerCase();
    return decks.filter((deck) => (active === 'All' || deck.category === active) && (!needle || `${deck.title} ${deck.description} ${deck.category} ${deck.slug}`.toLowerCase().includes(needle))).sort((a, b) => sort === 'az' ? a.title.localeCompare(b.title) : b.saves - a.saves);

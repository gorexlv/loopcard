...
export function MarketDeckPreview({ deck }: { deck: PublicDeck }) {
  ...
  return <section className="deck-reader" aria-label={chinese ? '预览卡片' : 'Preview cards'}>
    <div className="deck-reader-stage"><RecallCard key={card.id} prompt={card.prompt} visual={card.visual} heading={section?.heading} body={section?.body} category={deck.category} answerLabel={section?.title} flipped={flipped} onFlip={() => setFlipped(!flipped)} index={cardIndex + 1} chinese={chinese} />

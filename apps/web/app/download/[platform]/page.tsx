...
export async function generateMetadata({ params }: { params: Promise<{ platform: string }> }): Promise<Metadata> { const item = platforms[(await params).platform as keyof typeof platforms]; return item ? { title: `LoopCard for ${item.name}`, description: `LoopCard for ${item.name} is coming soon. Use the web app while the ${item.store} release is being prepared.` } : {}; }
export default async function DownloadPage({ params }: { params: Promise<{ platform: string }> }) { const item = platforms[(await params).platform as keyof typeof platforms]; if (!item) notFound(); return <><SiteHeader /><main><section className="page-hero"><span className="eyebrow">Coming soon</span><h1>LoopCard for<br />{item.name}.</h1><p>The public {item.store} release is being prepared. Use LoopCard on the web today, or join the mobile beta from this page when distribution opens.</p><div className="hero-actions"><Link className="button button-teal" href="/app">Use the web app →</Link><Link className="button button-ghost" href="/blog">Read the journal</Link></div></section></main><SiteFooter /></> }

...
export default async function AppPage({ searchParams }: { searchParams: Promise<{ error?: string }> }) {
  ...
  const [{ data: ownedRows }, { data: savedRows }] = await Promise.all([
  ...
  return <main className="workspace-page"><header className="workspace-header"><div><h1>My decks</h1></div><Link className="button button-teal" href="/app/decks/new">Create a deck</Link></header>{query.error && <div className="form-message form-error" role="alert">{query.error}</div>}<DeckSection title="Created" decks={owned} empty="No cards yet." /><DeckSection title="Saved" decks={saved} empty="No saved decks." market /></main>;
}
